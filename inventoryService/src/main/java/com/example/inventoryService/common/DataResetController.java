...
public class DataResetController {
    ...
    public ResponseEntity<String> reset() throws Exception {
        ...
        // 2. 시드 데이터 삽입
        stockRepository.saveAll(seedProducts());
    ...
    private List<Stock> seedProducts() {
        return List.of(
                Stock.of("짜장면",  8000,  5,  true,  false),
                Stock.of("짬뽕",   10000,  5,  true,  false),
                Stock.of("탕수육", 18000,  0,  true,  true),
                Stock.of("군만두",  6000,  20, false, false),
                Stock.of("한정판 세트", 35000, 1, true, false)
        );
    }
}